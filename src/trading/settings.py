...
class RuntimeSettings(BaseModel):
    ...
    demo_relaxed_candidate_validation: bool = False
...
def load_settings() -> AppSettings:
    ...
    if (more_opps := os.getenv("TRADING_DEMO_MORE_OPPORTUNITIES_ENABLED")) is not None:
        runtime_cfg["demo_more_opportunities_enabled"] = more_opps.lower() in ("true", "1", "yes")

    if (relaxed_val := os.getenv("TRADING_DEMO_RELAXED_CANDIDATE_VALIDATION")) is not None:
        runtime_cfg["demo_relaxed_candidate_validation"] = relaxed_val.lower() in ("true", "1", "yes")
